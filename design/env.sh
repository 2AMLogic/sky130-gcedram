...
# Safe to source from any directory. Defaults mirror the canonical pin in
# docs/pdk-pin.md (kept in sync by hand -- shell can't import
# sim/_evidence_common.py's Python constants).
#
...
if [[ -f "${PDK_ROOT}/${PDK}/libs.tech/xschem/xschemrc" ]]; then
  ...
  echo "sky130-gcedram: PDK not found at ${PDK_ROOT}/${PDK} -- see design/README.md for the install command (docs/pdk-pin.md)" >&2
fi
